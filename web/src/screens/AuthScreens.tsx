...
import { login, cadastrar, recuperarSenha, redefinirSenha, loginGoogle } from '../services/auth'
...
export const AuthScreens = ({ setScreen }: Props) => {
  ...
  async function handleGoogleCredential(response: { credential?: string }) {
    if (!response?.credential) { setErro('Não foi possível autenticar com o Google'); return }
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      // Largura acompanha o container (full-width, igual ao botão Entrar). GIS limita a 400px.
      const largura = Math.floor(Math.min(400, googleBtnRef.current.getBoundingClientRect().width || 320))
      id.renderButton(googleBtnRef.current, {
        type: 'standard', theme: 'outline', size: 'large', shape: 'pill',
        text: modo === 'cadastro' ? 'signup_with' : 'continue_with',
        logo_alignment: 'left', locale: 'pt-BR', width: largura,
      })
    }

    if (gid()) { render(); return }
    const existente = document.getElementById('gsi-script') as HTMLScriptElement | null
    if (existente) { existente.addEventListener('load', render); return () => existente.removeEventListener('load', render) }
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="p-8 shadow-strong">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-full bg-brand-blue text-white font-extrabold text-base py-3.5 min-h-[52px] shadow-soft active:scale-[0.98] transition-transform disabled:opacity-60 disabled:cursor-not-allowed mb-3"
            >
        ...
        {(modo === 'login' || modo === 'cadastro') && (
          <div className="mt-6 flex flex-col gap-3">
            ...
            {GOOGLE_CLIENT_ID ? (
              <div ref={googleBtnRef} className="w-full flex justify-center items-center min-h-[52px]" />
            ) : (
              <button disabled
                className="flex items-center justify-center gap-3 w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full px-6 py-3.5 font-bold text-base opacity-60 cursor-not-allowed shadow-soft min-h-[52px]">
